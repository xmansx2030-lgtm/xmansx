...
def setup_attendance_env(school, *, students_count: int = 5, period_started_minutes_ago: int = 10):
    ...
    period = BellPeriod.objects.create(
        school=school, bell_schedule=schedule, sequence=3,
        name="الحصة الثالثة", start_time=start, end_time=end,
    )
    ...
    return {
        "year": year, "grade": grade, "section": section, "students": students,
        "schedule": schedule, "period": period, "local_now": local_now,
    }
