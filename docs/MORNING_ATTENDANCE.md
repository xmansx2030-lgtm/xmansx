...
Morning attendance is separate from period attendance. `SchoolArrival` stores the first arrival per school/student/date, raw lateness, counted lateness after grace, status, source, and correction history.
...
Student profiles expose morning late counts/minutes separately from period late counts/minutes and use dated enrollment for historical display.
