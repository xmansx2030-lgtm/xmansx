...
export function ReportsPage() {
  ...
  return (
    <div className="space-y-5" data-testid="reports-page">
      ...
      <div className="flex gap-1 overflow-x-auto rounded-2xl border border-slate-200 bg-white p-1 shadow-sm" role="tablist">
        {([
          ["absence", "الغياب"],
          ["lateness", "التأخر"],
          ["referrals", "الإحالات للمرشد"],
        ] as const).map(([value, label]) => (
          <button key={value} type="button" role="tab" aria-selected={tab === value} onClick={() => setTab(value)} className={`min-h-11 shrink-0 rounded-xl px-4 text-sm font-black ${tab === value ? "bg-slate-950 text-white" : "text-slate-600 hover:bg-slate-100"}`}>{label}</button>
        ))}
      </div>
...
function AbsenceReport({ schoolId, filters, onPage, schoolType }: ReportProps) {
  ...
  return <ReportFrame controls={<><Select label="نوع الغياب" value={absenceType} onChange={(value) => { setAbsenceType(value); onPage(1); }} options={[["ALL", "الكل"], ["FULL", "يوم كامل"], ["PARTIAL", "جزئي"]]} /><Select label="حالة العذر" value={excuseType} onChange={(value) => { setExcuseType(value); onPage(1); }} options={[["ALL", "الكل"], ["UNEXCUSED", "دون عذر"], ["EXCUSED", "بعذر فقط"], ["MIXED", "مختلط"]]} /></>} query={report} summary={report.data && <div className="grid grid-cols-2 gap-2 sm:grid-cols-4"><MetricCard label={studentPluralLabel(schoolType)} value={report.data.summary.students} /><MetricCard label="أيام غياب كامل" value={report.data.summary.full_absence_days} tone="red" /><MetricCard label="أيام غياب جزئي" value={report.data.summary.partial_absence_days} tone="amber" /><MetricCard label="حصص دون عذر" value={report.data.summary.unexcused_absent_periods} tone="red" /></div>} table={report.data && <AbsenceTable rows={report.data.results} />} footer={report.data && <Pagination count={report.data.count} page={report.data.page} pageSize={report.data.page_size} onPage={onPage} />} />;
}

function LatenessReport({ schoolId, filters, onPage, schoolType }: ReportProps) {
  const [latenessType, setLatenessType] = useState("ALL");
  const [minOccurrences, setMinOccurrences] = useState(0);
  const [minMinutes, setMinMinutes] = useState(0);
  const report = useQuery({ queryKey: schoolScopedKey(schoolId, "reports", "lateness", filters, latenessType, minOccurrences, minMinutes), queryFn: ({ signal }) => getLatenessReport(filters, { latenessType, minOccurrences, minMinutes }, signal), enabled: schoolId > 0 });
  return <ReportFrame controls={<><Select label="مصدر التأخر" value={latenessType} onChange={setLatenessType} options={[["ALL", "الصباحي والحصص"], ["MORNING", "صباحي فقط"], ["PERIOD", "داخل الحصص فقط"]]} /><NumberFilter label="الحد الأدنى للمرات" value={minOccurrences} onChange={setMinOccurrences} /><NumberFilter label="الحد الأدنى للدقائق" value={minMinutes} onChange={setMinMinutes} /></>} query={report} summary={report.data && <div className="grid grid-cols-2 gap-2 sm:grid-cols-4"><MetricCard label={studentCountLabel(schoolType)} value={report.data.summary.students} /><MetricCard label="مرات التأخر الصباحي" value={report.data.summary.morning_occurrences} tone="amber" /><MetricCard label="دقائق التأخر الصباحي" value={report.data.summary.morning_minutes} /><MetricCard label="مرات تأخر الحصص" value={report.data.summary.period_occurrences} tone="amber" /></div>} table={report.data && <LatenessTable rows={report.data.results} />} footer={report.data && <Pagination count={report.data.count} page={report.data.page} pageSize={report.data.page_size} onPage={onPage} />} />;
}
...
function Select({ label, value, onChange, options }: { label: string; value: string; onChange: (value: string) => void; options: [string, string][] }) { return <label className="min-w-40 text-xs font-bold text-slate-600">{label}<select value={value} onChange={(event) => onChange(event.target.value)} className="mt-1 h-11 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm">{options.map(([option, text]) => <option key={option} value={option}>{text}</option>)}</select></label>; }
function NumberFilter({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) { return <label className="min-w-40 text-xs font-bold text-slate-600">{label}<input type="number" min={0} value={value} onChange={(event) => onChange(Math.max(Number(event.target.value), 0))} className="mt-1 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm" /></label>; }
function Pagination({ count, page, pageSize, onPage }: { count: number; page: number; pageSize: number; onPage: (page: number) => void }) { const pages = Math.max(Math.ceil(count / pageSize), 1); if (pages <= 1) return null; return <div className="flex items-center justify-center gap-3 print:hidden"><Button variant="secondary" disabled={page <= 1} onClick={() => onPage(page - 1)}>السابق</Button><span className="text-sm text-slate-600">صفحة {page} من {pages}</span><Button variant="secondary" disabled={page >= pages} onClick={() => onPage(page + 1)}>التالي</Button></div>; }

function TableShell({ children, empty }: { children: React.ReactNode; empty: boolean }) { return <section className="overflow-x-auto rounded-2xl border border-slate-200 bg-white shadow-sm">{empty ? <EmptyState title="لا توجد نتائج" description="غيّر الفلاتر أو الفترة ثم أعد المحاولة." compact /> : <table className="w-full min-w-[760px] text-sm">{children}</table>}</section>; }
