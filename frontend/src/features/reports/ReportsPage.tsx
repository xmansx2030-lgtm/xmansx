...
function ReferralsReport({ schoolId, filters, onPage, schoolType, schoolName, isCleared, onClear, onRestore }: ReportProps) {
  ...
  return (
    <ReportFrame<ReferralRow, ReferralReportSummary>
      ...
      summary={report.data && <div className="grid grid-cols-2 gap-2 sm:grid-cols-4"><MetricCard label="إجمالي الإحالات" value={report.data.summary.total} /><MetricCard label="جديدة" value={report.data.summary.new} tone="blue" /><MetricCard label="غير معيّنة" value={report.data.summary.unassigned} tone="amber" /><MetricCard label="عاجلة" value={report.data.summary.high_priority} tone="red" /></div>}
