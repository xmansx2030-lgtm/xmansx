        ...
        <section className="overflow-hidden rounded-[2rem] border border-white/10 bg-white shadow-[0_35px_90px_rgba(0,0,0,0.3)]">
          ...
          <form onSubmit={handleSubmit} className="p-5 sm:p-9" noValidate>
            {step === 1 ? <>
              ...
              <fieldset><legend className="mb-3 text-sm font-bold text-slate-700">اختر الباقة <span className="text-red-600">*</span></legend>{plans.isPending ? <div className="h-28 animate-pulse rounded-2xl bg-slate-100" /> : plans.data && plans.data.length > 0 ? <div className="grid gap-3 sm:grid-cols-2">{plans.data.map((plan) => { const isFree = Number(plan.price_amount) === 0; return <label key={plan.id} className={`relative flex min-h-32 cursor-pointer flex-col rounded-2xl border p-4 transition ${effectivePlanId === plan.id ? "border-teal-500 bg-teal-50 ring-2 ring-teal-100" : "border-slate-200 hover:border-slate-300"}`}><input type="radio" name="plan_id" value={plan.id} checked={effectivePlanId === plan.id} onChange={() => setPlanId(plan.id)} className="sr-only" /><span className="flex items-start justify-between gap-3"><strong>{plan.name}</strong>{effectivePlanId === plan.id && <span className="grid size-6 place-items-center rounded-full bg-teal-700 text-white"><Check size={14} /></span>}</span><span className="mt-2 text-xs leading-5 text-slate-500">{isFree ? "استخدام مجاني دون مدة تجربة" : `${plan.trial_days.toLocaleString("ar-SA")} يومًا للتجربة`}</span><span className="mt-auto pt-3 text-sm font-black text-teal-800">{isFree ? "مجانية" : `${Number(plan.price_amount).toLocaleString("ar-SA")} ${plan.currency}`}</span></label>; })}</div> : <Alert tone="warning" title="لا توجد باقة متاحة">التسجيل الذاتي متوقف مؤقتًا. تواصل مع الدعم للبدء.</Alert>}{fieldErrors.plan_id && <p role="alert" className="mt-2 text-sm text-red-700">{fieldErrors.plan_id}</p>}</fieldset>
