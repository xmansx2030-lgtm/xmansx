    ...
    <main className="landing-page min-h-dvh overflow-hidden bg-[#061916] text-white">
      ...
      <div id="landing-content">
        ...
        <section id="plans" className="scroll-mt-20 bg-[#f4f8f7] px-4 py-24 text-slate-900 sm:px-6 lg:px-8 lg:py-32"><div className="mx-auto max-w-7xl"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="text-sm font-black text-teal-700">ابدأ بتجربة فعلية</p><h2 className="mt-3 text-3xl font-black sm:text-5xl">باقة واضحة، دون مفاجآت</h2></div><p className="max-w-md text-sm leading-7 text-slate-600">الباقات المعروضة هنا تُجلب مباشرة من إعدادات المنصة، ويمكن لفريق التشغيل تحديثها دون تغيير الصفحة.</p></div>
          {plans.isPending ? <div className="mt-12 grid gap-5 md:grid-cols-2"><div className="h-72 animate-pulse rounded-[2rem] bg-slate-200" /><div className="h-72 animate-pulse rounded-[2rem] bg-slate-200" /></div> : plans.data && plans.data.length > 0 ? <div className="mt-12 grid gap-5 md:grid-cols-2 xl:grid-cols-3">{plans.data.map((plan, index) => <article key={plan.id} className={`relative flex flex-col rounded-[2rem] border bg-white p-7 shadow-sm ${index === 0 ? "border-teal-300 ring-4 ring-teal-100/60" : "border-slate-200"}`}>{index === 0 && <span className="absolute -top-3 right-7 rounded-full bg-teal-700 px-4 py-1.5 text-xs font-black text-white">الخيار المقترح</span>}<p className="text-sm font-black text-teal-700">{plan.name}</p><div className="mt-5 flex items-end gap-2"><strong className="text-4xl font-black">{Number(plan.price_amount).toLocaleString("ar-SA")}</strong><span className="mb-1 text-sm text-slate-500">{plan.currency} / {billingLabel(plan.billing_period)}</span></div><p className="mt-4 min-h-14 text-sm leading-7 text-slate-600">{plan.description || "كل ما تحتاجه مدرستك لبدء المواظبة والمتابعة من مساحة موحدة."}</p><div className="my-6 h-px bg-slate-100" /><ul className="flex-1 space-y-3">{planHighlights(plan).map((item) => <li key={item} className="flex items-center gap-2 text-sm font-semibold text-slate-700"><span className="grid size-6 place-items-center rounded-full bg-teal-50 text-teal-700"><Check size={14} /></span>{item}</li>)}</ul><Link to={`/register?plan=${plan.id}`} className="mt-7 inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-slate-950 px-5 text-sm font-black text-white hover:bg-teal-800">ابدأ تجربة {plan.trial_days.toLocaleString("ar-SA")} يومًا <ArrowLeft size={16} /></Link></article>)}</div> : <div className="mt-12 rounded-3xl border border-slate-200 bg-white p-8 text-center"><h3 className="text-xl font-black">سيتم إعلان الباقات قريبًا</h3><p className="mt-2 text-sm text-slate-600">تواصل معنا لتجهيز مدرستك وخطة التشغيل المناسبة.</p><a href={WHATSAPP_URL} target="_blank" rel="noreferrer" className="mt-5 inline-flex min-h-11 items-center rounded-xl bg-teal-700 px-5 text-sm font-black text-white">تواصل معنا</a></div>}
        </div></section>
