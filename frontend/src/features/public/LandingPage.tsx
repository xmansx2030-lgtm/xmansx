...
import {
  ...
  ChevronLeft, Fingerprint, GraduationCap, HeartHandshake, Menu,
  QrCode, ShieldCheck, Sparkles, UsersRound, X,
  type LucideIcon,
} from "lucide-react";
...
function planHighlights(plan: PublicPlan) {
  ...
  return [...items, "تشغيل كامل خلال التجربة", "دعم فني باللغة العربية"].slice(0, 4);
}
...
export function LandingPage() {
  ...
  return (
    <main className="landing-page min-h-dvh overflow-hidden bg-[#061916] text-white">
      ...
      <header className="fixed inset-x-0 top-0 z-50 border-b border-white/8 bg-[#061916]/86 backdrop-blur-xl">
        <nav aria-label="التنقل العام" className="mx-auto flex h-18 max-w-7xl items-center gap-6 px-4 sm:px-6 lg:px-8">
          ...
          <button type="button" className="ms-auto grid size-11 place-items-center rounded-xl border border-white/10 bg-white/5 sm:hidden" aria-label={mobileNavOpen ? "إغلاق القائمة" : "فتح القائمة"} aria-expanded={mobileNavOpen} onClick={() => setMobileNavOpen((value) => !value)}>{mobileNavOpen ? <X size={20} /> : <Menu size={20} />}</button>
        </nav>
        {mobileNavOpen && <div className="border-t border-white/8 bg-[#08211d] p-4 sm:hidden"><div className="grid gap-2 text-sm font-bold"><a onClick={() => setMobileNavOpen(false)} href="#capabilities" className="flex min-h-11 items-center rounded-xl px-3 hover:bg-white/5">المنصة</a><a onClick={() => setMobileNavOpen(false)} href="#journey" className="flex min-h-11 items-center rounded-xl px-3 hover:bg-white/5">كيف تبدأ</a><a onClick={() => setMobileNavOpen(false)} href="#plans" className="flex min-h-11 items-center rounded-xl px-3 hover:bg-white/5">الباقات</a><Link to="/login" className="flex min-h-11 items-center rounded-xl px-3 hover:bg-white/5">تسجيل الدخول</Link><Link to="/register" className="mt-2 flex min-h-12 items-center justify-center rounded-xl bg-teal-400 px-4 text-slate-950">ابدأ مدرستك</Link></div></div>}
      </header>

      <div id="landing-content">
        <section className="landing-hero relative isolate px-4 pb-24 pt-32 sm:px-6 sm:pb-32 sm:pt-40 lg:px-8">
        ...
        <section className="border-y border-white/8 bg-white/[0.025] px-4 py-7 sm:px-6"><div className="mx-auto flex max-w-7xl flex-wrap items-center justify-center gap-x-10 gap-y-4 text-sm font-bold text-slate-400"><span className="text-xs text-slate-500">مساحة واحدة لـ</span>{[[GraduationCap, "شؤون الطلاب"], [UsersRound, "الهيئة التعليمية"], [HeartHandshake, "الإرشاد الطلابي"], [BarChart3, "القيادة المدرسية"]].map(([Icon, label]) => { const ItemIcon = Icon as LucideIcon; return <span key={label as string} className="flex items-center gap-2"><ItemIcon size={18} className="text-teal-300" />{label as string}</span>; })}</div></section>
        ...
        <section id="capabilities" className="scroll-mt-20 bg-[#f4f8f7] px-4 py-24 text-slate-900 sm:px-6 lg:px-8 lg:py-32">
          <div className="mx-auto max-w-7xl">
            ...
            <div className="mt-12 grid gap-6 lg:grid-cols-[0.82fr_1.18fr]">
              <div role="tablist" aria-label="محاور المنصة" className="space-y-3">{FEATURES.map((feature) => { const Icon = feature.icon; const active = feature.id === activeFeature; return <button key={feature.id} type="button" role="tab" aria-selected={active} onClick={() => setActiveFeature(feature.id)} className={`flex min-h-20 w-full items-center gap-4 rounded-2xl border p-4 text-start transition ${active ? "border-teal-200 bg-white shadow-[0_18px_50px_rgba(15,118,110,0.1)]" : "border-transparent bg-transparent hover:border-slate-200 hover:bg-white/70"}`}><span className={`grid size-11 shrink-0 place-items-center rounded-xl ${active ? "bg-teal-700 text-white" : "bg-slate-200/70 text-slate-600"}`}><Icon size={20} /></span><span className="min-w-0 flex-1"><span className="block text-xs font-bold text-teal-700">{feature.eyebrow}</span><span className="mt-1 block font-black">{feature.title}</span></span><ChevronLeft size={18} className={active ? "text-teal-700" : "text-slate-300"} /></button>; })}</div>
              <article role="tabpanel" className="relative overflow-hidden rounded-[2rem] bg-[#09231f] p-6 text-white shadow-[0_30px_70px_rgba(7,27,25,0.2)] sm:p-10">
        ...
        <section id="journey" className="scroll-mt-20 px-4 py-24 sm:px-6 lg:px-8 lg:py-32"><div className="mx-auto max-w-7xl"><div className="text-center"><p className="text-sm font-black text-teal-300">ابدأ بلا تعقيد</p><h2 className="mt-4 text-3xl font-black sm:text-5xl">من التسجيل إلى التشغيل في رحلة واضحة</h2></div><div className="mt-14 grid gap-4 md:grid-cols-3">{[{ n: "01", t: "أنشئ المدرسة", d: "أدخل بيانات المدرسة والمدير، واختر الباقة العامة المناسبة." }, { n: "02", t: "اضبط التشغيل", d: "أكمل العام الدراسي والجداول، ثم استورد المعلمين والطلاب." }, { n: "03", t: "تابع بثقة", d: "ابدأ التحضير، راقب المؤشرات، واتخذ الإجراء من نفس المساحة." }].map((step) => <article key={step.n} className="group rounded-[1.75rem] border border-white/9 bg-white/[0.045] p-7 transition hover:-translate-y-1 hover:border-teal-300/25 hover:bg-white/[0.065]"><span className="text-sm font-black text-teal-300">{step.n}</span><h3 className="mt-8 text-xl font-black">{step.t}</h3><p className="mt-3 text-sm leading-7 text-slate-400">{step.d}</p></article>)}</div></div></section>

        <section id="plans" className="scroll-mt-20 bg-[#f4f8f7] px-4 py-24 text-slate-900 sm:px-6 lg:px-8 lg:py-32"><div className="mx-auto max-w-7xl"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="text-sm font-black text-teal-700">ابدأ بتجربة فعلية</p><h2 className="mt-3 text-3xl font-black sm:text-5xl">باقة واضحة، دون مفاجآت</h2></div><p className="max-w-md text-sm leading-7 text-slate-600">الباقات المعروضة هنا تُجلب مباشرة من إعدادات المنصة، ويمكن لفريق التشغيل تحديثها دون تغيير الصفحة.</p></div>
          {plans.isPending ? <div className="mt-12 grid gap-5 md:grid-cols-2"><div className="h-72 animate-pulse rounded-[2rem] bg-slate-200" /><div className="h-72 animate-pulse rounded-[2rem] bg-slate-200" /></div> : plans.data && plans.data.length > 0 ? <div className="mt-12 grid gap-5 md:grid-cols-2 xl:grid-cols-3">{plans.data.map((plan, index) => { const isFree = Number(plan.price_amount) === 0; const duration = formatPlanDuration(plan.duration_value, plan.duration_unit); return <article key={plan.id} className={`relative flex flex-col rounded-[2rem] border bg-white p-7 shadow-sm ${index === 0 ? "border-teal-300 ring-4 ring-teal-100/60" : "border-slate-200"}`}>{index === 0 && <span className="absolute -top-3 right-7 rounded-full bg-teal-700 px-4 py-1.5 text-xs font-black text-white">الخيار المقترح</span>}<p className="text-sm font-black text-teal-700">{plan.name}</p><div className="mt-5 flex items-end gap-2"><strong className="text-4xl font-black">{isFree ? "مجانية" : Number(plan.price_amount).toLocaleString("ar-SA")}</strong>{!isFree && <span className="mb-1 text-sm text-slate-500">{plan.currency}</span>}</div><p className="mt-3 inline-flex w-fit rounded-full bg-teal-50 px-3 py-1.5 text-sm font-black text-teal-800">مدة الباقة: {duration}</p><p className="mt-4 min-h-14 text-sm leading-7 text-slate-600">{plan.description || "كل ما تحتاجه مدرستك لبدء المواظبة والمتابعة من مساحة موحدة."}</p><div className="my-6 h-px bg-slate-100" /><ul className="flex-1 space-y-3">{planHighlights(plan).map((item) => <li key={item} className="flex items-center gap-2 text-sm font-semibold text-slate-700"><span className="grid size-6 place-items-center rounded-full bg-teal-50 text-teal-700"><Check size={14} /></span>{item}</li>)}</ul><Link to={`/register?plan=${plan.id}`} className="mt-7 inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-slate-950 px-5 text-sm font-black text-white hover:bg-teal-800">{isFree ? `ابدأ مجانًا لمدة ${duration}` : `ابدأ تجربة ${plan.trial_days.toLocaleString("ar-SA")} يومًا`} <ArrowLeft size={16} /></Link></article>; })}</div> : <div className="mt-12 rounded-3xl border border-slate-200 bg-white p-8 text-center"><h3 className="text-xl font-black">سيتم إعلان الباقات قريبًا</h3><p className="mt-2 text-sm text-slate-600">تواصل معنا لتجهيز مدرستك وخطة التشغيل المناسبة.</p><a href={WHATSAPP_URL} target="_blank" rel="noreferrer" className="mt-5 inline-flex min-h-11 items-center rounded-xl bg-teal-700 px-5 text-sm font-black text-white">تواصل معنا</a></div>}
        </div></section>
