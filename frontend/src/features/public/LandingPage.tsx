        ...
        <section id="plans" className="scroll-mt-20 bg-[#f4f8f7] px-4 py-24 text-slate-900 sm:px-6 lg:px-8 lg:py-32">
          <div className="mx-auto max-w-7xl">
            <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
              <div><p className="text-sm font-black text-teal-700">اختر ما يناسب مدرستك</p><h2 className="mt-3 text-3xl font-black sm:text-5xl">جميع الباقات، بصورة واضحة</h2></div>
              <p className="max-w-md text-sm leading-7 text-slate-600">تُعرض هنا كل الباقات العامة النشطة من إعدادات المنصة، مع توضيح ما يمكن تجربته مباشرة وما يتطلب طلب اشتراك.</p>
            </div>
            {plans.isPending ? (
              <div className="mt-12 grid gap-5 md:grid-cols-2"><div className="h-72 animate-pulse rounded-[2rem] bg-slate-200" /><div className="h-72 animate-pulse rounded-[2rem] bg-slate-200" /></div>
              ...
              <div className="mt-12 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                {plans.data.map((plan, index) => {
                  ...
                  return (
                    <article key={plan.id} className={`relative flex flex-col rounded-[2rem] border bg-white p-7 shadow-sm ${index === 0 ? "border-teal-300 ring-4 ring-teal-100/60" : "border-slate-200"}`}>
                      ...
                      {plan.can_self_register ? (
                        <Link to={`/register?plan=${plan.id}`} className="mt-7 inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-slate-950 px-5 text-sm font-black text-white hover:bg-teal-800">{isFree ? `ابدأ مجانًا لمدة ${duration}` : `ابدأ تجربة ${plan.trial_days.toLocaleString("ar-SA")} يومًا`} <ArrowLeft size={16} /></Link>
                      ) : (
                        <a href={planContactUrl(plan.name)} target="_blank" rel="noreferrer" className="mt-7 inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-slate-950 px-5 text-sm font-black text-white hover:bg-teal-800">اطلب هذه الباقة <ArrowLeft size={16} /></a>
                      )}
                    </article>
                  );
                })}
