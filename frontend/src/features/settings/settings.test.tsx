...
describe("SettingsPage", () => {
  ...
  it("creates and activates the first academic year in one guided action", async () => {
    const createdYear = {
      id: 41,
      name: "2026/2027",
    ...
    const { calls } = mockApi({
      "/auth/me/": { body: meWithRoles(["SCHOOL_MANAGER"]) },
      "/school/academic-years/": (init) => ({
        body: init?.method === "POST" ? createdYear : [],
        status: init?.method === "POST" ? 201 : 200,
      }),
      "/school/academic-years/41/activate/": {
        body: { ...createdYear, status: "ACTIVE" },
      },
    });
    ...
    expect(
      await screen.findByText(/سيُنشأ العام ويُفعّل مباشرةً في خطوة واحدة/),
    ).toBeInTheDocument();
    await user.type(screen.getByLabelText(/اسم العام/), "2026/2027");
    await user.type(screen.getByLabelText("بداية العام"), "2026-08-23");
    await user.type(screen.getByLabelText("نهاية العام"), "2027-06-25");
    await user.click(screen.getByRole("button", { name: "إنشاء العام وتفعيله" }));

    await waitFor(() => {
      expect(
        calls.some(
          (call) =>
            call.url.endsWith("/school/academic-years/") && call.init?.method === "POST",
        ),
      ).toBe(true);
      expect(
        calls.some((call) => call.url.includes("/academic-years/41/activate/")),
      ).toBe(true);
    });
    expect(await screen.findByRole("status")).toHaveTextContent(
      "تم إنشاء العام 2026/2027 وتفعيله",
    );
  });

  it("creates and activates the first semester of an active year", async () => {
    const activeYear = {
      id: 51,
      name: "2026/2027",
      start_date: "2026-08-23",
    ...
    const { calls } = mockApi({
      "/auth/me/": { body: meWithRoles(["SCHOOL_MANAGER"]) },
      "/school/academic-years/51/semesters/": { body: createdSemester, status: 201 },
      "/school/semesters/61/activate/": {
        body: { ...createdSemester, status: "ACTIVE" },
      },
      "/school/academic-years/": { body: [activeYear] },
    });
    renderApp("/settings?section=calendar");
    const user = userEvent.setup();

    await user.click(await screen.findByRole("button", { name: "إضافة فصل دراسي" }));
    await user.type(screen.getByLabelText("البداية"), "2026-08-23");
    await user.type(screen.getByLabelText("النهاية"), "2026-12-31");
    await user.click(screen.getByRole("button", { name: "حفظ الفصل وتفعيله" }));

    await waitFor(() => {
      expect(
        calls.some(
          (call) =>
            call.url.includes("/academic-years/51/semesters/") &&
            call.init?.method === "POST",
        ),
      ).toBe(true);
      expect(calls.some((call) => call.url.includes("/semesters/61/activate/"))).toBe(
        true,
      );
    });
  });
