...
export function StudentLeavesTab({ studentId }: { studentId: number }) {
  ...
  if (leaves.data.results.length === 0) return <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center"><DoorOpen aria-hidden size={28} className="mx-auto text-slate-300" /><p className="mt-3 font-bold text-slate-700">لا توجد استئذانات مسجلة لهذا الطالب.</p></div>;
